...
import {
  CARD_MARGIN,
  DEFAULT_CARD_SIZE,
} from '../album-card/album-card.component';
import {
  IPageInfo,
...
export class AlbumsListComponent implements OnInit, AfterViewInit {
  ...
  private calculateColumns() {
    if (this.listElement) {
      const width =
        this.listElement.nativeElement.getBoundingClientRect().width - 20;
      this.columnsCount = Math.floor(width / (DEFAULT_CARD_SIZE + CARD_MARGIN));

      const freeSpace =
        width - this.columnsCount * (DEFAULT_CARD_SIZE + CARD_MARGIN);
